...
import { initialize, uninitialize, transferDirect, transferWithSwap } from "../utils/apiRequests";
...
describe("borderless", () => {
  ...
  // const fullPrivateKey = adminKeypair.secretKey;
  // const bs58 = require('bs58');
  // const privateKeyBase58 = bs58.encode(fullPrivateKey);
  // console.log("Base58 Encoded Private Key:", privateKeyBase58);
  // // Convert the private key to a Base64 string
  // const privateKeyBase64 = Buffer.from(fullPrivateKey).toString('base64');
  // console.log("Base64 Encoded Private Key:", privateKeyBase64);
  ...
  it("Uninitialize!", async () => {
    const txHash = await uninitialize(program);
    console.log("uninitialize", txHash);
  });

  it("Initialize!", async () => {
    const txHash = await initialize(program);
